...
1.  **Context Bundle Analysis**
    ...
    - Sync dependencies once per checkout to keep Enaible tooling aligned with the repository:

      ```bash
      uv sync --project tools/enaible
      ```

    - Capture OpenCode activity with the Enaible CLI, applying optional UUID and search-term filters:
    ...
    - Aggregate cross-repo activity when required:
...
2.  **Git Status Review**
    ...
    - Inspect uncommitted work and categorize change types before mapping them to session activity:

      ```bash
      git status --short
      ```

    - Correlate modified files with recent OpenCode sessions to surface active areas.
...
3.  **Git History Review**
    ...
    - Record architectural shifts, renames, regressions, and follow-up TODOs gathered from these commands.

4.  **Contributor & Pattern Mapping**

    - Identify ownership and recurring problem themes:
    ...
    - Pair contributor focus with session objectives to highlight collaboration points and repeated issues.

5.  **Generate Consolidated Summary**
    - Merge bundle insights, git status, and history into a cohesive narrative tailored for OpenCode workflows.
    - Extract file access frequencies, semantic search matches, operation patterns, and outstanding risks to steer next actions.
    - Recommend follow-up checks (tests, stakeholders) and spotlight the most active files for hand-offs or onboarding.
    - When `--search-term` is present, expand the capture with domain-aware variations to broaden recall while preserving chronology.
