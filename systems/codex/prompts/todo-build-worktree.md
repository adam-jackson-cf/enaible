# Todo Implementation Program (imported from https://github.com/badlogic/codex-commands)
...
1. Check for task resume: If `task.md` exists in current directory:
   - Read `task.md` and `AGENTS.md` in full in parallel
...
3. Read `Codex.md` in full
   ...
   - If missing:
     ...
     - Write confirmed content to `AGENTS.md`
...
4. Check if project description needs updating:
   - If implementation changed structure, features, or commands:
     - Present proposed updates to `AGENTS.md`
...
7. STOP → "PR created. Delete the worktree? (y/n)"
   ...
   - Note: If Codex was spawned in the worktree, the working directory will become invalid after removal
