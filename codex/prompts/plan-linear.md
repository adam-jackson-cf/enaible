...
1. Explicit `--config`
2. Project-level `.codex/linear-plan.config.json`
3. User-level `$HOME/.codex/linear-plan.config.json`
...
```json
{
  ...
  "error": {
    ...
    "details": {
      ...
      "attempted_sources": [
        "--config (not provided)",
        ".codex/linear-plan.config.json",
        "$HOME/.codex/linear-plan.config.json"
      ]
    }
  }
}
