...
4. **Generate PRD Structure**
   ...
   **FIRST - Resolve SCRIPT_PATH:**
   ...
   1. **Try project-level .codex folder**:
      ...
      ```bash
      Glob: ".codex/scripts/plan/generate_prd.py"
      ```

   2. **Try user-level .codex folder**:

      ```bash
      Bash: ls "$HOME/.codex/scripts/plan/generate_prd.py"
      ```

   3. **Interactive fallback if not found**:
      - List searched locations: `.codex/scripts/generators/` and `$HOME/.codex/scripts/generators/`
