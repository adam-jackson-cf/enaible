...
- Guard against Copilot’s own follow-up PRs. Add `if: ${{ github.actor != 'app/copilot-swe-agent' }}` and label only Copilot-authored PRs with `copilot-doc-review` so the workflow can skip those branches without blocking human reruns.
...
- Copilot-authored PRs (e.g., branch `copilot/audit-docs-for-changes`) still need human approval to run Actions; expect the yellow “Workflows awaiting approval” banner and have a maintainer press “Approve workflows to run”. Label those PRs automatically so the “skip recursion” guard can identify them while leaving human PRs unaffected.
