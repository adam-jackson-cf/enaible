...
- Do not invoke external analyzer scripts; rely on file listing, search, and git inspection.
- Support every claim with specific evidence (file:line and command used).
...
1. Scope & Setup
   ...
   - Initial commands:
     - `ls -1A "$TARGET_PATH"`
     - `rg --files --hidden ${EXCLUDE_ARG}` (where `${EXCLUDE_ARG}` is `--glob '!{...}'` when provided)
     - Inspect key manifests: `package.json`, `tsconfig.json`, `pyproject.toml`, `tailwind.config.*`, `drizzle.config.*`
...
2. Repository Survey & Index
   ...
   - Representative queries (apply `$EXCLUDE_GLOBS` and `$TARGET_PATH`):
     - Hono: `rg -n --hidden "from\\s+\"hono\"|require\(\"hono\"\)" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - Router: `rg -n --hidden "@tanstack/(react-router|router)|react-router-dom|routeTree\\.gen\\.ts" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - State/Data: `rg -n --hidden "from\\s+\"zustand\"|@tanstack/(react-query|query-core)" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - DB/ORM: `rg -n --hidden "drizzle|drizzle-orm|better-sqlite3|drizzle\\.config\\.(ts|js)" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - UI/Tokens: `rg -n --hidden "@radix-ui/react-|shadcn/ui|tailwind\\.config\\.(ts|js|cjs)" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - Analytics/Identity: `rg -n --hidden "posthog-(js|node)|clerk|@clerk|stripe|braintree|better-auth" "$TARGET_PATH" ${EXCLUDE_ARG}`
     - CI: `ls -1A "$TARGET_PATH/.github/workflows" 2>/dev/null`
