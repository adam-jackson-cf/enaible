...
class FallbackStrategy:
    ...
    async def execute(
        self, context: ErrorContext, original_function: Callable, *args, **kwargs
    ) -> Any:
...
class TraditionalCrawlFallback(FallbackStrategy):
    ...
    async def execute(
        ...
        logger.warning(
            f"Falling back to traditional crawling due to: {context.message}"
        )
        ...
        max_pages = min(
            args[2] if len(args) > 2 else kwargs.get("max_budget", 20), 15
        )  # Reduced budget
...
class SimpleSummaryFallback(FallbackStrategy):
    ...
    async def execute(
        ...
        logger.warning(
            f"Falling back to simple summarisation due to: {context.message}"
        )
...
class KeywordBasedRelevanceFallback(FallbackStrategy):
    ...
    async def execute(
        ...
        logger.warning(
            f"Falling back to keyword-based relevance due to: {context.message}"
        )
...
class ErrorHandler:
    ...
    def _classify_error(self, error: Exception, operation: str) -> ErrorContext:
        ...
        if any(
            ...
            for keyword in ["connection", "network", "timeout", "dns"]
        ):
        ...
        # LLM sampling errors
            ...
            keyword in error_str
            for keyword in ["llm", "sampling", "model", "openai", "anthropic"]
        ):
        ...
        return ErrorContext(
            ...
            operation=operation,
            timestamp=time.time(),
            metadata={"error_class": type(error).__name__, "original_error": error},
    ...
    def _calculate_backoff_delay(
        self, retry_count: int, error_type: ErrorType
    ) -> float:
    ...
    async def handle_with_retry(
        ...
        for attempt in range(self.config.llm_models.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    logger.info(
                        f"Successfully recovered {operation} after {attempt} retries"
                    )
    ...
    def get_error_statistics(self) -> dict[str, Any]:
        ...
        recent_errors = [
            err for err in self.error_history if time.time() - err.timestamp < 3600
        ]
...
async def handle_with_resilience(
    operation: str, func: Callable[..., Awaitable[Any]], *args, **kwargs
) -> Any:
    """Handle operations with resilience and retry logic."""
...
def resilient_operation(operation_name: str):
    """Make functions resilient with automatic retry and fallback."""
