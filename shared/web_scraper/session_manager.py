...
class SessionManager:
    ...
    async def cleanup_old_sessions(self):
        ...
        async with self._lock:
            ...
            if (
                len(self._sessions)
                > self._config.crawler_settings.max_concurrent_sessions
            ):
                inactive_sessions = [
                    sid for sid, active in self._sessions.items() if not active
                ]

                # Clean up oldest inactive sessions, keep some buffer
                cleanup_threshold = (
                    self._config.crawler_settings.session_cleanup_threshold
                )
                cleanup_buffer = self._config.crawler_settings.session_cleanup_buffer
                sessions_to_cleanup = (
                    inactive_sessions[:-cleanup_buffer]
                    if len(inactive_sessions) > cleanup_threshold
                    else []
                )

                for session_id in sessions_to_cleanup:
                    try:
                        if self._crawler and hasattr(
                            self._crawler.crawler_strategy, "kill_session"
                        ):
                            await self._crawler.crawler_strategy.kill_session(
                                session_id
                            )
                            logger.info(f"Cleaned up old session: {session_id}")
                    except Exception as e:
                        logger.warning(
                            f"Error cleaning up old session {session_id}: {e}"
                        )
                    finally:
                        if session_id in self._sessions:
