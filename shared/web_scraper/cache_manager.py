...
class LRUCache:
    ...
    def put(self, key: str, value: Any, content_hash: str | None = None):
        """Put value in cache."""
        ...
        # Add new entry
        entry = CacheEntry(
            value=value, timestamp=time.time(), content_hash=content_hash
        )
...
class CacheManager:
    ...
    def __init__(self):
        ...
        self.relevance_cache = (
            ...
            if cache_config.enable_relevance_cache
            else None
        )
        ...
        self.summary_cache = (
            ...
            if cache_config.enable_summary_cache
            else None
        )
        ...
        self.content_hash_cache = (
            ...
            if cache_config.enable_content_deduplication
            else None
        )
        ...
        logger.info(
            f"Initialized cache manager with relevance={self.relevance_cache is not None}, "
            f"summary={self.summary_cache is not None}, "
            f"deduplication={self.content_hash_cache is not None}"
        )

    # Relevance caching
    def get_relevance_score(self, content_hash: str, query: str) -> float | None:
    ...
    def cache_relevance_score(self, content: str, query: str, score: float):
        ...
        logger.debug(
            f"Cached relevance score {score:.3f} for content hash {content_hash[:8]}"
        )

    # Summary caching
    def get_summary(
        self, content_hash: str, max_tokens: int, focus: str | None = None
    ) -> str | None:
        """Get cached summary."""
    ...
    def cache_summary(
        self, content: str, summary: str, max_tokens: int, focus: str | None = None
    ):
        """Cache summary."""
        if not self.summary_cache:
            return
    ...
    def find_similar_content(self, content: str, url: str) -> tuple[str, float] | None:
        ...
        for cached_hash, entry in self.content_hash_cache.cache.items():
            ...
            if hasattr(entry.value, "content"):
                similarity = self.hasher.content_similarity(
                    content, entry.value["content"]
                )
                if (
                    similarity > self.config.content_similarity_threshold
            ...
            if hasattr(entry.value, "url"):
                ...
                if (
                    url_sim > self.config.url_similarity_threshold
                    and url_sim > best_similarity
                ):
                    best_match = cached_hash
