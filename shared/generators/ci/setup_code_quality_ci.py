#!/usr/bin/env python3
"""
Generate GitHub Actions workflow for Code Quality using project analyzers + converters.

Outputs .github/workflows/codex-code-quality.yml
 - Runs analyzers via uv + Enaible project
 - Converts analyzer JSONs to CodeClimate JSON array gl-code-quality-report.json
...
def _generate_workflow(
    fail_on: str, paths_include: list[str], comment_on_pr: bool
) -> str:
    paths_yaml = _paths_block(paths_include)
    comment_default = "true" if comment_on_pr else "false"
    template = """name: Codex Code Quality
    ...
    steps:
      ...
      - name: Install tools in Enaible env
        run: |
          ...
          uv run --project tools/enaible python -m pip install lizard
          npm i -g jscpd
...
{_filters_yaml()}
      ...
      - name: Run quality:lizard
        if: steps.changes.outputs.code == 'true'
        run: |
          PYTHONPATH=shared uv run --project tools/enaible \
            enaible analyzers run quality:lizard \
            --target . --out artifacts/quality_lizard.json --min-severity high

      - name: Run quality:jscpd
        if: steps.changes.outputs.code == 'true'
        run: |
          PYTHONPATH=shared uv run --project tools/enaible \
            enaible analyzers run quality:jscpd \
            --target . --out artifacts/quality_jscpd.json --min-severity high

      - name: Run quality:pattern_classifier
        if: steps.changes.outputs.code == 'true'
        run: |
          PYTHONPATH=shared uv run --project tools/enaible \
            enaible analyzers run quality:pattern_classifier \
            --target . --out artifacts/quality_patterns.json --min-severity high

      - name: Convert to CodeClimate JSON
        if: steps.changes.outputs.code == 'true'
        run: |
          PYTHONPATH=shared uv run --project tools/enaible \
            enaible ci convert-codeclimate artifacts \
      ...
      - name: Comment summary on PR
        ...
        with:
          script: |
            ...
            const files = ['artifacts/quality_lizard.json','artifacts/quality_jscpd.json','artifacts/quality_patterns.json'];
            let crit=0, high=0;
            ...
            const body = [
              ...
              `- High: ${high}`,
              '',
              'Artifacts include gl-code-quality-report.json for CodeClimate consumers.'
            ].join('\n');
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Code Quality GitHub Actions workflow"
    )
    parser.add_argument(
        "--fail-on",
    ...
    wf = workflows / "codex-code-quality.yml"
    ...
    content = _generate_workflow(args.fail_on, args.paths_include, comment_on_pr)
    wf.write_text(content, encoding="utf-8")

    print("✅ Created .github/workflows/codex-code-quality.yml")
