...
### [Code Quality CI/CD Job Example](#code-quality-cicd-job-example)
...
### [Step 1: Generating Recommendations](#step-1-generating-recommendations)
...
#### [Security Recommendations CI/CD Job Example](#security-recommendations-cicd-job-example)
...
### [Example Output: Consolidated SAST Findings](#example-output-consolidated-sast-findings)
...
Repo leak enables minting valid admin JWTs; trivial exploitation, internet-facing.
...
Login endpoint; product search

Critical

Raw SQL concatenation; direct login bypass and data exfiltration via public HTTP handlers.

3

CWE-94

Server-side code injection via eval
...
`eval()` on user input allows RCE; conditionally enabled but still high-impact when reachable.

4

— (SSRF)

SSRF via arbitrary image URL fetch

1

Image URL fetch/write flow

High

Outbound fetch of unvalidated URLs enables internal service / metadata access (e.g., AWS metadata).

#### [Top 5 Immediate Actions](#top-5-immediate-actions)
...
##### [Deduplicated Findings (Full Details)](#deduplicated-findings-full-details)

##### [1\. CWE-798 — Hardcoded JWT private key](#1-cwe-798--hardcoded-jwt-private-key)
...
##### [2\. CWE-89 — SQL injection (login & search)](#2-cwe-89--sql-injection-login--search)
...
##### [3\. CWE-94 — Server-side code injection (`eval`)](#3-cwe-94--server-side-code-injection-eval)
...
##### [4\. SSRF — Arbitrary image URL fetch](#4-ssrf--arbitrary-image-url-fetch)
...
### [Step 2: Remediating Security Issues Based on Recommendations](#step-2-remediating-security-issues-based-on-recommendations)
...
#### [Remediation CI/CD Job Example](#remediation-cicd-job-example)
