...
# Removes AI-Assisted Workflows components while preserving .opencode directory structure
...
ARGUMENTS:
    TARGET_PATH     Directory containing .opencode/ to uninstall from
                   Examples:
...
DESCRIPTION:
    ...
    the .opencode directory structure and any user-added files:

    - Removes workflow command files
    - Removes agent prompt files
    - Removes analysis scripts
    - Removes rule files
    - Cleans up auxiliary files (configs, tooling)
    - Removes sections from agents.md
...
# Find and validate .opencode installation
find_opencode_installation() {
    # Set target path for .opencode directory
    if [[ -z "$TARGET_PATH" ]]; then
        TARGET_PATH="$(pwd)"
    fi

    # Handle path ending with .opencode
    if [[ "$TARGET_PATH" == */.opencode ]]; then
        OPENCODE_DIR="$TARGET_PATH"
        log_verbose "Target path already ends with .opencode, using it directly"
    else
        OPENCODE_DIR="$TARGET_PATH/.opencode"
        log_verbose "Looking for .opencode in target path"
    fi

    log "Checking for AI-Assisted Workflows installation at: $OPENCODE_DIR"

    if [[ ! -d "$OPENCODE_DIR" ]]; then
        log_error "No .opencode directory found at: $OPENCODE_DIR"
        echo "Please specify the correct path containing your .opencode installation"
        exit 1
    fi
    ...
    if [[ -f "$OPENCODE_DIR/command/plan-solution.md" ]]; then
        ((our_files_found++))
    fi

    # Check for our scripts
    if [[ -d "$OPENCODE_DIR/scripts/core" ]] || [[ -d "$OPENCODE_DIR/scripts/analyzers" ]]; then
        ((our_files_found++))
    fi

    # Check for our rules
    if [[ -f "$OPENCODE_DIR/rules/global.opencode.rules.md" ]]; then
        ((our_files_found++))
    fi

    if [[ $our_files_found -eq 0 ]]; then
        log_error "No AI-Assisted Workflows components found at: $OPENCODE_DIR"
...
remove_command_files() {
    ...
    if [[ ! -d "$OPENCODE_DIR/command" ]]; then
        log_verbose "No command directory found"
        return 0
    fi

    # List of our command files (OpenCode has 19 commands)
    local our_commands=(
        ...
        "analyze-security.md"
        "apply-rule-set.md"
        "codify-opencode-history.md"
        "create-project.md"
        "create-rule.md"
        "create-session-notes.md"
        "get-primer.md"
        "get-recent-context.md"
        "improve-gpt5-prompt.md"
        "plan-refactor.md"
        "plan-solution.md"
        "plan-ux-prd.md"
    ...
    for cmd in "${our_commands[@]}"; do
        if [[ -f "$OPENCODE_DIR/command/$cmd" ]]; then
            files_to_remove+=("$cmd")
        fi
    done
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        for file in "${files_to_remove[@]}"; do
            if rm "$OPENCODE_DIR/command/$file" 2>/dev/null; then
...
remove_agent_files() {
    ...
    if [[ ! -d "$OPENCODE_DIR/agent" ]]; then
        log_verbose "No agent directory found"
        return 0
    fi
    ...
    for agent in "${our_agents[@]}"; do
        if [[ -f "$OPENCODE_DIR/agent/$agent" ]]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local removed_count=0
        for file in "${files_to_remove[@]}"; do
            if rm "$OPENCODE_DIR/agent/$file" 2>/dev/null; then
        ...
        if [[ -d "$OPENCODE_DIR/agent" ]] && [[ -z "$(ls -A "$OPENCODE_DIR/agent")" ]]; then
            rmdir "$OPENCODE_DIR/agent" 2>/dev/null && log_verbose "Removed empty agent directory"
        fi
...
remove_script_directories() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local removed_count=0
        for item in "${items_to_remove[@]}"; do
            if rm -rf "$OPENCODE_DIR/$item" 2>/dev/null; then
        ...
        if [[ -d "$OPENCODE_DIR/scripts" ]]; then
            find "$OPENCODE_DIR/scripts" -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true
        ...
        if [[ -d "$OPENCODE_DIR/scripts" ]]; then
            ...
            if [[ -z "$(find "$OPENCODE_DIR/scripts" -type f)" ]]; then
                rm -rf "$OPENCODE_DIR/scripts" 2>/dev/null && log_verbose "Removed empty scripts directory"
            fi
        fi
...
remove_rule_files() {
    ...
    for rule in "${our_rules[@]}"; do
        if [[ -f "$OPENCODE_DIR/rules/$rule" ]]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        local removed_count=0
        for file in "${files_to_remove[@]}"; do
            if rm "$OPENCODE_DIR/rules/$file" 2>/dev/null; then
        ...
        if [[ -d "$OPENCODE_DIR/rules" ]] && [[ -z "$(ls -A "$OPENCODE_DIR/rules")" ]]; then
            rmdir "$OPENCODE_DIR/rules" 2>/dev/null && log_verbose "Removed empty rules directory"
        fi
    else
...
# Remove sections from agents.md
remove_agents_md_sections() {
    log "Checking for our sections in agents.md..."

    if [[ ! -f "$OPENCODE_DIR/agents.md" ]]; then
        log_verbose "No agents.md file found"
        return 0
    fi

    # Check if our "Build Approach Flags" section exists
    if ! grep -q "## Build Approach Flags for claude enhanced workflows" "$OPENCODE_DIR/agents.md"; then
        log "No AI-Assisted Workflows sections found in agents.md"
        return 0
    fi

    echo ""
    echo "Found AI-Assisted Workflows sections in agents.md:"
    echo "  - Build Approach Flags section"
    echo ""

    if [[ "$DRY_RUN" == "true" ]]; then
        log "Would remove sections from agents.md"
        return 0
    fi

    read -p "Remove our sections from agents.md? (y/n): " -n 1 -r
    echo

    if [[ $REPLY =~ ^[Yy]$ ]]; then
        # Create backup of agents.md
        local backup_file="${OPENCODE_DIR}/agents.md.backup.$(date +%Y%m%d_%H%M%S)"
        cp "$OPENCODE_DIR/agents.md" "$backup_file"
        ...
        rm -f "$OPENCODE_DIR/agents.md.tmp"

        log "Removed AI-Assisted Workflows sections from agents.md"
        echo "Backup saved as: $(basename "$backup_file")"
    else
        log "Skipped removing agents.md sections"
    fi
}

# Read installation log for better removal warnings
read_installation_log() {
    local log_file="$OPENCODE_DIR/installation-log.txt"
...
remove_python_packages() {
    ...
    if [[ -f "$OPENCODE_DIR/scripts/setup/requirements.txt" ]]; then
        requirement_files+=("$OPENCODE_DIR/scripts/setup/requirements.txt")
    fi
    if [[ -f "$OPENCODE_DIR/scripts/setup/ci/requirements.txt" ]]; then
        requirement_files+=("$OPENCODE_DIR/scripts/setup/ci/requirements.txt")
    fi
...
show_summary() {
    ...
    echo ""
    echo "Installation location: $OPENCODE_DIR"
    echo ""
    ...
    echo "  ✓ agents.md sections checked/removed"
    echo "  ✓ Python packages offered for removal"
    echo ""
    echo "The .opencode directory structure has been preserved."
...
main() {
    ...
    echo "  $OPENCODE_DIR"
    echo ""
    echo "The .opencode directory and any user-added files will be preserved."
    echo ""
