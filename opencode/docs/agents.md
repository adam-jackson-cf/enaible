...
_Scraped from https://opencode.ai/docs/agents/ on 2025-09-29_
...
- JSON in your `opencode.json`
- Markdown files placed in agent directories
...
Here are all the tools that can be controlled through the agent config.
...
- Common permissions: edit, bash, webfetch
...
```
{  "$schema": "https://opencode.ai/config.json",  "permission": {    "edit": "ask",    "bash": "allow",    "webfetch": "deny"  }}
```
...
```
{  "$schema": "https://opencode.ai/config.json",  "agent": {    "build": {      "permission": {        "edit": "allow",        "bash": {          "*": "allow",          "git push": "ask",          "terraform *": "deny"        },        "webfetch": "ask"      }    }  }}
```
...
```
{  "$schema": "https://opencode.ai/config.json",  "permission": {    "bash": {      "*": "allow",      "git push": "ask",      "terraform *": "deny"    }  }}
```

If you provide a granular bash map, the default becomes ask unless you set `*` explicitly.

```
{  "$schema": "https://opencode.ai/config.json",  "permission": {    "bash": {      "git status": "allow"    }  }}
```
...
```
{  "$schema": "https://opencode.ai/config.json",  "permission": { "bash": "ask" },  "agent": {    "build": {      "permission": {        "bash": { "git status": "allow", "*": "ask" }      }    }  }}
```
...
```
{  "$schema": "https://opencode.ai/config.json",  "agent": {    "review": {      "permission": { "edit": "deny", "bash": "deny", "webfetch": "allow" }    }  }}
```

Planning agent that can browse but cannot change code

```
{  "$schema": "https://opencode.ai/config.json",  "agent": {    "plan": {      "permission": { "edit": "deny", "bash": "deny", "webfetch": "ask" }    }  }}
```
...
> **Tip**: Do you have an agent you’d like to share? [Submit a PR](https://github.com/sst/opencode).
...
_Edit this page_: https://github.com/sst/opencode/edit/dev/packages/web/src/content/docs/agents.mdx
_Find a bug? Open an issue_: https://github.com/sst/opencode/issues/new
_Join our Discord community_: https://opencode.ai/discord
