...
function Show-Usage {
    ...
    Write-Output "                     ~\                    (User global: ~\.opencode\)"
    ...
    Write-Output ""
    Write-Output "    # Install globally for user (creates ~\.opencode\)"
    Write-Output "    .\install.ps1 ~"
    Write-Output ""
...
function Handle-ExistingInstallation {
    ...
    if (Test-Path $OpenCodePath) {
        ...
        if ($InstallMode -eq "Interactive") {
            ...
            Write-Output ""
            Write-Output "Found existing .opencode directory at: $OpenCodePath"
        ...
        switch ($choice) {
            "1" {
                ...
                Remove-Item -Path $OpenCodePath -Recurse -Force
                return @{ BackupPath = $backupPath; Mode = "fresh" }
            }
            "2" {
...
function Install-ESLintPackages {
    ...
    $packageJson = @{
        name = "opencode-eslint-workspace"
        version = "1.0.0"
        description = "ESLint workspace for OpenCode"
        private = $true
        dependencies = @{
            "eslint" = "^8.0.0"
        ...
        switch ($InstallMode) {
            "fresh" {
                ...
                Invoke-WithSpinner -Message "Copying shared scripts" -ScriptBlock {
                    Copy-SharedScripts -OpenCodePath $OpenCodePath
                }
            }
            "merge" {
                ...
                if (-not (Test-Path (Join-Path $OpenCodePath "scripts"))) {
                    Copy-SharedScripts -OpenCodePath $OpenCodePath
                }
            ...
            "update-workflows" {
                ...
                $templatesPath = Join-Path $OpenCodePath "templates"
                $sourceTemplatesPath = Join-Path $sourceOpenCodeDir "templates"
                ...
                $sourceRulesPath = Join-Path $sourceOpenCodeDir "rules"
...
function Show-CompletionMessage {
    ...
    Write-Output "  1. Try a command: opencode /analyze-security"
    Write-Output "  2. Use build flags: --prototype or --tdd"
    Write-Output ""
    ...
    Write-Output "  • Documentation: $OpenCodePath\README.md"
...
function Handle-GlobalRules {
    ...
    if (Test-Path $targetOpenCodeFile) {
        ...
        Add-Content -Path $targetOpenCodeFile -Value "---"
        Add-Content -Path $targetOpenCodeFile -Value ""
        Add-Content -Path $targetOpenCodeFile -Value "# AI-Assisted Workflows v$VERSION - Auto-generated, do not edit"
        Add-Content -Path $targetOpenCodeFile -Value ""

        $rulesContent = Get-Content $sourceRulesFile
        Add-Content -Path $targetOpenCodeFile -Value $rulesContent
...
try {
    ...
    $installResult = Handle-ExistingInstallation $OpenCodePath
    ...
    Copy-WorkflowFiles $OpenCodePath $installMode
    ...
    if (-not (Test-Installation $OpenCodePath)) {
