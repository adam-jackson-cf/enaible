...
handle_global_rules() {
    local source_dir="$1"
    local source_rules_md="$source_dir/rules/global.opencode.rules.md"
    local target_agents_md="$INSTALL_DIR/agents.md"

    # Check if our source global.opencode.rules.md exists
    ...
    if [[ -f "$target_agents_md" ]]; then
        ...
        if grep -q "# AI-Assisted Workflows v" "$target_agents_md" 2>/dev/null; then
        ...
        # No existing agents.md, copy ours with header
        log_verbose "No existing agents.md found, creating new one with global rules..."
        {
            echo "# AI-Assisted Workflows v$SCRIPT_VERSION - Auto-generated, do not edit"
            echo ""
            cat "$source_rules_md"
        } > "$target_agents_md"
        log "Created agents.md with global rules in installation directory"
    fi
}
...
show_completion() {
    ...
    echo "  Build Flags: --prototype, --tdd (via opencode.md)"
    echo ""
    echo "Usage examples:"
