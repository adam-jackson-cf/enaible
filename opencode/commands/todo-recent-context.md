...
1. **Context Bundle Analysis**
   ...
   - Find and analyze context bundles from the last 2 days in `.opencode/agent/context_bundles/`
...
ls -la .opencode/agent/context_bundles/ 2>/dev/null | grep -E "$(date +'%a_%d'|tr '[:lower:]' '[:upper:]')|$(date -d 'yesterday' +'%a_%d'|tr '[:lower:]' '[:upper:]')"

# With --uuid: Filter to specific session
ls -la .opencode/agent/context_bundles/*_${UUID}.json 2>/dev/null
```
